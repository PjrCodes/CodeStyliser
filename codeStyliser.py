...
def stylise_code(file_to_edit):
    ...
    while line_index < (len(lines) - 1):
        try:
            ...
            print(f"WARN: Found a comment inside Parentheses in {file_to_edit.name} around line {line_index+1}, \
            skipping line!!")
            continue
        except exceptions.HashIgnore:
            # currently, hash ignore output handled in handleKeyword
            continue
    ...
    if is_file_given:
        ...
        if file_ext == "c" or file_ext == "h":
            try:
                ...
                print(f"ERROR: Given filename, {given_file_name} not found!")
                sys.exit()
            except UnicodeDecodeError as e:
                print("ERROR: while decoding file " + given_file_name + " the file is NOT a UTF-8 encoded file,\
                Skipping file...")
                print(e)
                sys.exit()
            except (KeyboardInterrupt, SystemExit):
